...
export function checkUnreadInSearchResult() {
  ...
  if (items.length === 0) {
    ...
    for (const link of links) {
      ...
      if (result.diag && result.diag.length > 0) linkDiag.push(`${name}: ${result.diag.join('; ')}`);
    }
    return { hasUnread: false, debug: `no .list-group-item, links: [${linkDiag.join(' | ')}]` };
  }
  ...
  for (const item of items) {
    ...
    if (result.hasUnread) return result;
    if (result.diag && result.diag.length > 0) diag.push(`${name}: ${result.diag.join('; ')}`);
  }
  return { hasUnread: false, debug: diag.length > 0 ? diag.join(' | ') : 'no colored elements found' };
}
