...
export function scrollChatList(delta) {
  const el = document.querySelector('div.flex-fill.overflow-y-auto');
  if (el) { el.scrollTop += delta; return el.scrollTop; }
  return null;
}
...
/** 検索結果の最初のチャットに「今日」の活動があるか（チャットを開かずに判定） */
export function checkTodayActivityInSearchResult() {
  const panel = document.querySelector('div.flex-fill.overflow-y-auto');
  if (!panel) return false;
  const items = panel.querySelectorAll('.list-group-item');
  for (const item of items) {
    ...
    const h6 = item.querySelector('h6');
    if (!h6) continue;
    // この item が最初のユーザー検索結果
    const leafNodes = item.querySelectorAll('*');
    for (const el of leafNodes) {
      if (el.childElementCount > 0) continue;
      const txt = el.textContent.trim();
      if (!txt || txt.length > 20) continue;
      // 「今日」「HH:MM」（時刻のみ＝今日）を検出
      if (txt === '今日') return true;
      // 時刻のみ（昨日/曜日なしの HH:MM）は今日の活動
...
export function scanKarisateiTargets() {
  ...
  const allEls = el.querySelectorAll('*');
  for (const elem of allEls) {
    // 日付ラベルの検出: リーフノード（<a> 内外を問わず）
    // ただし長いテキスト（メッセージプレビュー）や名前（h6）は除外
    if (elem.childElementCount === 0 && elem.tagName !== 'H6' && elem.tagName !== 'P') {
      const txt = elem.textContent.trim();
      if (txt && txt.length <= 15) {
        if (txt === '昨日' || txt === yesterdayWeekday || txt === yesterdayShort ||
          ...
          currentDateLabel = '昨日';
          hasYesterday = true;
        } else if (txt === '今日' || /^\d{1,2}:\d{2}$/.test(txt)) {
          // 時刻形式（14:32）は「今日」として扱う
          if (!currentDateLabel) currentDateLabel = '今日';
        } else if (/^(?:月|火|水|木|金|土|日)曜日$/.test(txt) && txt !== yesterdayWeekday) {
          currentDateLabel = txt;
          hasOlderDates = true;
        } else if (/^\d{1,2}\/\d{1,2}$/.test(txt) && txt !== yesterdayShort) {
          currentDateLabel = txt;
          hasOlderDates = true;
        }
      }
    ...
    if (elem.tagName === 'A' && !processedLinks.has(elem)) {
      ...
      // フォールバック: currentDateLabel が未設定なら <a> の全テキストから判定
      let effectiveLabel = currentDateLabel;
      if (!effectiveLabel) {
        if (text.indexOf('昨日') > -1) effectiveLabel = '昨日';
        else if (text.indexOf(yesterdayWeekday) > -1) effectiveLabel = '昨日';
        else if (text.indexOf(yesterdayShort) > -1) effectiveLabel = '昨日';
        else if (/\d{1,2}:\d{2}/.test(text)) effectiveLabel = '今日';
      }
      if (sampleItems.length < 3) {
        sampleItems.push(`[${effectiveLabel || '?'}] ${text.substring(0, 80).replace(/\s+/g, ' ').trim()}`);
      }
      if (effectiveLabel === '昨日' &&
          text.indexOf('本査定のご案内') > -1 &&
