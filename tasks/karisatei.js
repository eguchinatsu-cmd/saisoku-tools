...
export async function runKarisatei(tabId, popupWindowId, logger) {
  ...
  try {
    await cdpScrollChatListToTop(tabId);
  } catch (e) {
...
async function scrollToYesterday(tabId, logger) {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    // CDP Runtime.evaluate経由で下スクロール（背面タブでも動作）
    const pos = await cdpScrollChatList(tabId, 400);
    // 10回ごとにスクロール位置を診断出力
    if (i % 10 === 0) {
      logger.info(`[scroll ${i}] scrollTop=${pos?.scrollTop}, scrollHeight=${pos?.scrollHeight}, clientHeight=${pos?.clientHeight}`);
    }
    await sleep(500);
  }
...
async function scanTargets(tabId, logger) {
  ...
  // CDP経由で少し上（今日寄り）に戻す
  for (let i = 0; i < 3; i++) {
    await cdpScrollChatList(tabId, -500);
    await sleep(400);
  }
  ...
  for (let scan = 0; scan < 100; scan++) {
    ...
    // CDP経由で下スクロール
    await cdpScrollChatList(tabId, 400);
    await sleep(600);
  }
  return [...allTargets];
}
...
async function processUser(tabId, userName, tagName, sentUsers, logger) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    // CDP経由でスクロール（上下交互）
    await cdpScrollChatList(tabId, attempt % 2 === 0 ? 400 : -400);
    await sleep(500);
  }
